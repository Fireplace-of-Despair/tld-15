    ...
    IHashingService hashingService) : PageModel, IPageAdmin
{
    ...
    {
        Id = "Introduction",
        LocalUrl = "/introduction/edit"
    };

    [BindProperty]
    public AFeatureIntroduction.ResponseRead? Model { get; set; }

    public async Task OnGetAsync()
    {
        var model = await mediator.Send(new AFeatureIntroduction.RequestRead());
        Model = model ?? new AFeatureIntroduction.ResponseRead
        {
